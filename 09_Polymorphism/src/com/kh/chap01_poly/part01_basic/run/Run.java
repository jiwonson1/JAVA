...
public class Run {
	...
	public static void main(String[] args) {
		...
		*/
		...
		ArrayList<Parent> list = new ArrayList<>(4);
		list.add(new Child1());
		list.add(new Child2());
		list.add(new Child1());
		list.add(new Child2());
		...
		for(int i=0; i<arr.length; i++) {
			...
			//if(i == 0 || i == 3) {
			/*
			if(arr[i] instanceof Child1) { // 해당 인덱스가 실제 Child1을 참조하고 있으면 true, 그게 아니면 false
			...
			*/
			...
			arr[i].print(); // 동적바인딩 : 컴파일시에는 레퍼런스 타입의 클래스에 기술된 메소드로 바인딩 되지만 
